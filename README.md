...
- **Reviewer agent (triangular orchestration)** (`review` tool + SessionEnd auto-review + opt-in PreCompact review): spawn a reviewer subagent that reads git diff against the feature description and returns a structured verdict (pass/fail/scope_creep/quality_issue). Long-running autonomous sessions don't silently drift — the reviewer catches "tests green but implementation off-intent" before `done`.
