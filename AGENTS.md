...
> Routing file for AI coding agents working on the **loom** project. Keep this short (≤ 100 lines). Project-specific details live in `docs/`.

## Project

**loom** — minimal Claude-Code-like coding agent in Python (≈ 300 LOC). Provides an `agent_loop` with tool use, hooks, context compression, and subagent delegation.
...
uv run python -m loom.cli run # Start the agent REPL
uv run python -m loom.cli eval               # Run the eval suite (text)
uv run python -m loom.cli eval --html r.html # Run eval + write HTML report
uv run python -m loom.cli eval --benchmark resume # Run resume canary (Phase 5 §6)
uv run python -m loom.cli trace show         # Show recent trace events
uv run python -m loom.cli audit              # Audit this project's harness
uv run python -m loom.cli init .             # Scaffold harness in target dir
scripts/verify-quick.sh                      # Fast dev-cycle verify (auto scope, ~5-10s)
uv run python -m loom.cli eval --filter foo  # Run a single eval case by substring
```
...
| Path | Purpose |
|---|---|
| `loom/agent/loop.py` | `agent_loop()`, message flow, checkpoint, trace |
| `loom/agent/context.py` | Token estimation, `microcompact`, `autocompact` |
| `loom/agent/hooks.py` | `Hooks` registry, deny list, permission rules |
| `loom/agent/llm.py` | `LLMClient` (Anthropic SDK wrapper) |
| `loom/agent/prompt.py` | `SystemPrompt` (static + dynamic, cache boundary) |
| `loom/agent/tools.py` | `run_*` tool handlers, `spawn_subagent` |
| `loom/agent/trace.py` | Append-only JSONL trace (`.minicode/trace.jsonl`) |
| `loom/agent/checkpoint.py` | Atomic save of conversation state |
| `loom/eval/` | EvalCase framework + 32 cases (init/audit/detect/memory/skills/integration) |
| `loom/cli.py` | Argparse + subcommand routing |
...
7. **Review→Rule**: When a review or post-mortem surfaces a recurring failure pattern, promote it to a numbered Working Rule here AND, when cheap, encode it as an eval case under `loom/eval/cases/`. One-off fixes don't compound; rules do.
8. **Eval cases are first-class tests**: New features that change observable behavior should ship with at least one eval case in `loom/eval/cases/`. The eval runner is the regression net for product behavior; pytest is for unit correctness.
9. **Monkey-patches need explicit import wiring**: A `monkey-patch = X` assignment in a module body fires **only when that module is imported**. If a patch file (e.g. `loom/tui/kitty_patch.py`) patches a class but is never imported from the application entry point, the patch is dead code — unit tests pass (because they import the patch module directly) but production behavior is unchanged. Verify with a startup print or `pid` log written from the patch module itself. The fix is one `import` line, but the diagnosis can take hours.
...
13. **textual.widgets.Markdown's `gfm-like` parser turns file names into bogus URLs**: it enables `linkify-it`, which matches `domain.tld` against the public-suffix list. Any file name whose extension is a real TLD (`conftest.py`, `setup.sh`, `README.md`, `a.py`, anything ending in `.dev`, `.io`, `.ai`, etc.) is silently re-rendered as `http://conftest.py` and becomes clickable — clicking opens the default browser at a non-existent domain. Fix: pass a `parser_factory` to every `Markdown` subclass that builds `MarkdownIt("gfm-like")` and sets `parser.options["linkify"] = False`. The chat log does this in `_markdown_parser_factory` and threads it through `UserMessage`, `AssistantMessage`, `StreamingOverlay`, `ThinkingDisplay`, and the `CollapsibleToolOutput` Markdown instances. `tests/test_markdown_linkify.py` guards the factory, the wiring, and the end-to-end "no link" invariant.
14. **TUI thinking display only updates for the first LLM call in a session, not subsequent ones** (after tool_use rounds). The agent loop fires `on_message_start` exactly **once** per `agent_loop()` invocation (i.e. once per user turn), but a single user turn can contain many LLM calls — the first LLM call's thinking was set up correctly, but the 2nd, 3rd, ... Nth LLM call's thinking was silently appended to the now-hidden `ThinkingDisplay` from the first round, so the user never saw it. Fix: add a new `on_assistant_message_start` callback that fires **before each LLM call** within the agent loop's while loop. The TUI wires it to the same `AssistantTurnStart` → `show_thinking_spinner` flow that resets `_thinking_reasoning` and creates a fresh `ThinkingDisplay`. The existing `on_message_start` keeps its once-per-session semantic (still guarded by the eval case in `loom/eval/cases/async_streaming.py`); the new `on_assistant_message_start` is per-LLM-call. `tests/test_thinking_per_llm_call.py` guards the callback wiring and the per-round fresh-display invariant.
15. **"Same as X" user requests mean full behavioral parity, not just visible fixes**: when a user says "design intent is the same as ThinkingMarker / pattern Y", they mean **complete behavioral parity** — every interaction (on_click, on_press, chain counts, modal/popup paths, hover states, keyboard handlers) must match, not just the visible symptom they reported. Symptoms are usually the **first** thing they noticed, not the **whole** bug. Concretely: before declaring done on a "parity" request, grep the reference widget's interaction surface (`grep -n "def on_click\|def on_press\|push_screen\|ModalScreen\|chain" loom/tui/<ref>.py`) and assert your fix removes every divergent path. The `f-tool-display-p2` post-mortem (2026-06-22): user said "click should expand like thinking content"; first fix only addressed the blank-render symptom and left double-click-modal path intact; user came back saying "still broken — double-click still shows the card". Second pass removed the modal path entirely. Encode the parity check as an eval case (`TuiToolCallMarkerClickTogglesOutput` now asserts `_open_modal` does NOT exist + chain 1/2/3 all toggle).
16. **Prefer `Static` over `Markdown` for inline content**: Markdown's `update()` returns `AwaitComplete` not awaited from sync code — async parse+mount races with display-toggle/layout-recalc. `f-tool-display-p2` (2026-06-22): content went blank after agent turn end. Static's `update()` is sync. **Rule**: inline tool output / status → `Static(markup=False)`; user/assistant messages → `Markdown`. `Vertical` default is `height: 1fr` — with `max-height`, add `height: auto`.
17. **Two-tier verification — dev cycle uses `scripts/verify-quick.sh` (~5-10s), closeout uses `./init.sh` + `loom eval` (~165s)**: `./init.sh` runs ALL 563 pytest + 255 eval every time, which dominates wall time during the dev cycle and tempts you to skip it. The quick script (a) auto-infers which `tests/test_*.py` matches your `git diff` and runs only that file, (b) skips `snap_compare` snapshot tests via the auto-applied `snapshot` marker, (c) extracts `name = "..."` strings from changed `loom/eval/cases/*.py` and runs only those cases. **Rule**: while iterating on a feature, run `scripts/verify-quick.sh` after every change; reserve full `./init.sh` + `loom eval` for closeout (status → done) and any time you suspect a cross-module regression. If you need a single eval case: `uv run python -m loom.cli eval --filter <substring>`. If you need a specific test file: `scripts/verify-quick.sh tests/test_foo.py`.

### Cache strategy: AGENTS.md is loaded into SystemPrompt.static if ≤ 12000 chars (then prompt-cached); larger files fall back to Tier 2.

### Cold-start continuity: session-handoff.md (full) and the last 80 lines of progress.md are auto-loaded into Tier 1.5 of the system prompt — no need for the agent to read them manually.
...
2. Run `loom eval` if any eval case was touched — must be 32/32 (or more)
...
uv run python -m loom.cli run    # type a question, expect streamed response
```

## Escalation

- **Architecture decision**: Read `docs/architecture.md` first. If still unclear, ask user.
- **Tool behavior unclear**: Read `docs/tools.md` + the `run_*` functions in `loom/agent/tools.py`.
- **Hook ordering / events**: Read `docs/hooks.md` and `loom/agent/hooks.py`.
- **Compression heuristics**: Read `docs/context.md` and the constants in `loom/agent/context.py`.
- **Trace schema or eval case design**: Read `loom/agent/trace.py` and `loom/eval/runner.py` before adding cases.
