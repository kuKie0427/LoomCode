"""Tests for _ctx_rail_components — §2.2.3 primitive 1 shuttle pass on ctx rail. Returns (shuttle_x, colorized_rail_str).
...
def test_ctx_rail_idle_zero_ratio():
    ...
    _, out = _ctx_rail_components(0.0, 0, "idle")
    assert out.count("\u25cf") == 1
    # Position 0: ●─────...
...
def test_ctx_rail_idle_full_ratio():
    ...
    _, out = _ctx_rail_components(1.0, 0, "idle")
    assert out.count("\u25cf") == 1
...
def test_ctx_rail_idle_freezes_at_phase_zero():
    ...
    _, out_phase0 = _ctx_rail_components(0.5, 0, "idle")
    _, out_phase1 = _ctx_rail_components(0.5, 1, "idle")
...
def test_ctx_rail_active_phase_zero():
    ...
    _, out = _ctx_rail_components(0.5, 0, "executing")
    import re
...
def test_ctx_rail_active_phase_one():
    ...
    _, out = _ctx_rail_components(0.5, 1, "executing")
    import re
...
def test_ctx_rail_color_thresholds():
    ...
    _, out_ok = _ctx_rail_components(0.5, 0, "executing")
    _, out_warn = _ctx_rail_components(0.7, 0, "executing")  # >= 0.60 warn threshold
    _, out_err = _ctx_rail_components(0.9, 0, "executing")  # >= 0.85 danger threshold
...
def test_ctx_rail_clamps_out_of_range_ratio():
    ...
    _, out_neg = _ctx_rail_components(-0.1, 0, "executing")
    _, out_over = _ctx_rail_components(1.5, 0, "executing")
    import re
    ...
    for ratio in (0.0, 0.3, 0.5, 0.7, 0.9, 1.0):
        for phase in (0, 1):
            for state in ("idle", "executing"):
                _, out = _ctx_rail_components(ratio, phase, state)
                assert "\u2588" not in out, (
