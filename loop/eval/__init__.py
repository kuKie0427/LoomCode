...
The eval cases live under `loop.eval.cases`. They are auto-discovered on
first import via `loop.eval.cases.__init__`. Each case subclasses
...
def run_evals(workdir=None, html_output=None) -> int:
    ...
    if html_output is not None:
        ...
        path.write_text(html_report(passed, results, title="Loop Eval Report"), encoding="utf-8")
