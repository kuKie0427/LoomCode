...
class TuiPackageImports(EvalCase):
    ...
    def run(self) -> EvalResult:
        try:
            import loop.tui  # noqa: F811, F401
            from loop.tui.app import AgentTUIApp  # noqa: F401
...
class TuiAppCallsApplyConfigAndSessionEnd(EvalCase):
    ...
    def run(self) -> EvalResult:
        with patch("loop.agent.loop.apply_config") as mock_apply, \
             patch("loop.agent.loop.hooks") as mock_hooks, \
             patch("loop.agent.user_hooks.discover_user_hooks", return_value={}), \
             patch("loop.agent.loop._active_config") as mock_config:
...
class TuiLaunchesInTestMode(EvalCase):
    ...
    def run(self) -> EvalResult:
        ...
        async def _pilot_test() -> None:
            with patch("loop.agent.user_hooks.discover_user_hooks", return_value={}):
                app = AgentTUIApp()
