...
class CheckpointRoundtripPreservesToolUseBlocks(EvalCase):
    ...
    def run(self) -> EvalResult:
        ...
        from loop.agent.checkpoint import load, save
...
class CheckpointLoadReturnsNoneForCorruptJson(EvalCase):
    ...
    def run(self) -> EvalResult:
        ...
        from loop.agent.checkpoint import default_path_for, load
        from loop.eval._util import make_empty_workdir
...
class CheckpointLoadReturnsNoneForMissingFile(EvalCase):
    ...
    def run(self) -> EvalResult:
        ...
        from loop.agent.checkpoint import load
        from loop.eval._util import make_empty_workdir
...
class CheckpointResumeCliRestoresHistory(EvalCase):
    ...
    def run(self) -> EvalResult:
        ...
        proc = subprocess.run(
            [sys.executable, "-m", "loop.cli", "run", "--resume"],
            cwd=wd,
            input="exit\n",
            capture_output=True,
            text=True,
