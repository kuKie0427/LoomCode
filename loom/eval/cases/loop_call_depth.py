...
class LoopCallDepthEnforcedAtMax(EvalCase):
    ...
    def run(self) -> EvalResult:
        ...
        proc = subprocess.run(
            [sys.executable, "-m", "loom.cli", "audit", "--help"],
...
class LoopCallDepthIncrementsAcrossCalls(EvalCase):
    name = "loom-call-depth-increments-across-calls"
    description = "Setting LOOP_CALL_DEPTH=1 in a parent python then invoking 'loom' results in the loom subprocess seeing depth=1"
    ...
    def run(self) -> EvalResult:
        ...
        proc = subprocess.run(
            [sys.executable, "-c",
             ...
             "p = subprocess.run([sys.executable, '-m', 'loom.cli', 'audit', '--help'], "
