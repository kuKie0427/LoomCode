...
def _seed_minimal_harness(workdir) -> None:
    """Populate a workdir with the bare minimum harness so `loom audit` returns >0.
...
class AuditTextMentionsAllSubsystems(EvalCase):
    ...
    description = "loom audit text output mentions all 5 subsystems"
...
class AuditJsonIsValid(EvalCase):
    ...
    description = "loom audit --json produces valid JSON with overall + bottleneck"
...
class AuditHtmlIsValid(EvalCase):
    ...
    description = "loom audit --html writes valid HTML containing the 5 subsystems"
...
class AuditExitsNonZeroWhenBelowMin(EvalCase):
    ...
    description = "loom audit exits non-zero when overall < min-score"
