...
class ScopeCheckCliInvocationWarns(EvalCase):
    ...
    description = "End-to-end: 'loom audit' in a tmpdir with 2 in-progress → WIP=1 warning in CLI output (stdout)"
    ...
    def run(self) -> EvalResult:
        ...
        try:
            ...
            proc = subprocess.run(
                [sys.executable, "-m", "loom.cli", "audit", "--skip-self-test", "--min-score", "0", str(wd)],
