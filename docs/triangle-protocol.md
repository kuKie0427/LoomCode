...
| # | Invariant | Enforcement type | Enforced by (eval case + code) |
|---|---|---|---|
| I1 | Every `task()` call MUST be followed by `run_review()` before the feature's status becomes `done` | hard | `feature_list.json` state machine: `done` transition requires a `pass` verdict in `review_attempts` history. Eval: `review-state-machine-*` (Phase TP-2) — `triangle-protocol-` does not directly cover I1 (relies on review-state-machine-schema-has-review-pending + review-session-end-fires-when-active-feature) |
| I2 | Reviewer's tool list MUST NOT contain `task` or `review` (no self-review, no nested review) | hard | `REVIEW_TOOLS` tuple in `tools.py:1348`. Eval: `triangle-protocol-no-self-review` + `review-tool-not-in-sub-tools` |
| I3 | Generator's tool list MUST NOT contain `task` or `review` (no re-delegation) | hard | `SUB_TOOLS` list in `tools.py:1248`. Eval: `triangle-protocol-no-self-review` (verifies absence in both lists) |
| I4 | `<delta_report>` MUST be the last block in Generator's final message (closing `</delta_report>` tag followed only by whitespace or EOF) | soft → hard | TP-2: wrapper around `spawn_subagent` checks return string, appends `<system-reminder>` on violation. TP-8: graduates to `HookAbort` (hard) via graduated response (regenerate-once → soft → hard). Eval: `triangle-protocol-sub-system-references-protocol` (mentions block) + `triangle-protocol-delta-report-roundtrip` (parse contract) |
| I5 | `<verdict>` and `<feedback_directive>` MUST both be present in Reviewer's final message | hard | `run_review()` post-processing: `parse_verdict()` + `parse_feedback_directive()`; missing → returns `verdict=unknown` + `fd=None`. Eval: `triangle-protocol-review-system-references-protocol` + `triangle-protocol-feedback-directive-roundtrip` + `triangle-protocol-run-review-injects-delta-report` |
| I6 | `feedback_directive.action` list MUST be non-empty and contain only valid actions per §7.3 combination rules | hard | `run_review()` post-processing: `parse_feedback_directive()` validates list non-empty + combination rules (§7.6 edge cases); invalid → returns `fd=None`. Eval: `triangle-protocol-action-list-validation-rules` (5 verdict↔action pairs) + `triangle-protocol-feedback-directive-roundtrip` |
| I7 | `delta_report.files_modified` MUST match `git diff --numstat` within ±10% per file | hard | `run_review()` pre-processing: calls `validate_delta_against_git_diff()` BEFORE spawning Reviewer; if violations non-empty, returns `verdict=unknown` with summary listing violations — bypasses LLM round-trip. Eval: `triangle-protocol-delta-mismatches-git-diff-detected` (validator) + `triangle-protocol-run-review-pre-validation-git-diff-bypasses-llm` (I7 hard bypass) |
| I8 | `delta_report.files_modified` paths MUST all fall within `scope_envelope.allow_paths` and not match `deny_paths` | hard | `run_review()` pre-processing: calls `validate_delta_against_scope()` BEFORE spawning Reviewer; if violations non-empty, returns `verdict=unknown` with summary listing violations — bypasses LLM round-trip. Eval: `triangle-protocol-delta-violates-scope-detected` (validator) + `triangle-protocol-run-review-pre-validation-scope-bypasses-llm` (I8 hard bypass) |
| I9 | Orchestrator MUST escalate after 3 consecutive non-pass verdicts on the same feature | hard | `feature_list.json` per-feature `review_attempts: int` counter: incremented on each `run_review()` non-pass verdict, reset to 0 on `pass`. Orchestrator reads counter; `review_attempts >= 3` → forced `escalate`. Counter survives autocompact and cross-session (persisted in JSON, not prompt context). Eval: `triangle-protocol-feedback-retry-bounded` (prompt + schema) + `triangle-protocol-trace-review-attempt-bounded-and-feedback-ends-chain` (trace attempt monotonicity) |
| I10 | `feedback_directive.retry_review=true` → Orchestrator MUST call `review` again after acting | soft | Orchestrator system prompt (TP-4) + trace event `triangle.feedback` records the action. Eval: `triangle-protocol-trace-review-attempt-bounded-and-feedback-ends-chain` (asserts feedback action=[none] ends chain — no more triangle.review events) |
| I11 | Each `task()` and `review()` tool call MUST emit a `triangle.delegate` / `triangle.review` trace event with `role: "generator" \| "reviewer"` field | hard | `trace.py` extension: events recorded in outer `task` / `review` tool handlers (NOT inside `spawn_subagent`, which would conflate Generator and Reviewer). Eval: `triangle-protocol-orchestrator-knows-roles` (role names) + `triangle-protocol-trace-delegate-paired-with-delta` (delegate→delta pairing + C8 fix) |
| I12 | PreCompact review verdict injected as `[system-reminder]` SHOULD be recognized by Orchestrator (not answered as user chat) | soft | Orchestrator system prompt (TP-4 Task 1) instructs the LLM; trace event `triangle.feedback` verifies Orchestrator eventually processed the verdict. Not a code-enforced invariant — LLM may misrecognize on edge cases. Eval: `triangle-protocol-pre-compact-verdict-recognized` (prompt check) |
